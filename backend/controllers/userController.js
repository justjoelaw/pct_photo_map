...
const deleteUser = asyncHandler(async (req, res) => {
  ...
  // Does the user still have JournalEntrys
  const journalEntry = await JournalEntry.findOne({ user: id }).lean().exec();
  if (journalEntry) {
    return res.status(400).json({ message: 'User has assigned JournalEntrys' });
  }
